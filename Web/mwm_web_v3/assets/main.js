...
// Contact form context and anti-spam timing
const contactForm = document.getElementById("contact-form");
if (contactForm) {
  const startedField = document.getElementById("form_started");
  if (startedField) startedField.value = Math.floor(Date.now() / 1000);

  const requestedOffer = new URLSearchParams(window.location.search).get("offer");
  const offerField = document.getElementById("offer");
  if (requestedOffer && offerField) {
    const match = [...offerField.options].find(
      (option) => option.value.toLowerCase() === requestedOffer.toLowerCase(),
    );
    if (match) offerField.value = match.value;
  }
}
...
if (overlay && zoomImg && zoomLink) {
  ...
  document.querySelectorAll(".project-thumb").forEach((thumb) => {
    thumb.addEventListener("click", () => openZoom(thumb));
...
function startParticles(canvas, cfg) {
  ...
  class Particle {
    ...
    reset() {
      ...
      this.radius =
        Math.random() * (cfg.radiusMax - cfg.radiusMin) + cfg.radiusMin;
      this.speedY =
        Math.random() * (cfg.speedMax - cfg.speedMin) + cfg.speedMin;
