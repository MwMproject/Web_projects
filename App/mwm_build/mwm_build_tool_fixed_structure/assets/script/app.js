...
pageNameInput.addEventListener("change", (event) => {
  currentPage().name = event.target.value || "Page sans titre";
  renderPageList();
});
...
function render() {
  ...
  pageNameInput.value = currentPage().name;
  ...
  emptyState.hidden = currentSections().length > 0;
  pagePreview.innerHTML = currentSections().map(renderSection).join("");
  bindSectionEvents();
  renderInspector();
  renderPageList();
  renderProjectList();
...
function runAction(action, id) {
  ...
  if (action === "delete") {
    ...
    state.selectedId = sections[index]?.id || sections[index - 1]?.id || null;
  }
...
function renderInspector() {
  const section = currentSections().find((item) => item.id === state.selectedId);
  if (!section) {
...
function newProject() {
  ...
  state.pages = [{ id: "page-home", name: "Accueil", sections: [] }];
  state.theme = {
    primary: "#2563eb",
    secondary: "#f97316",
    background: "#f8fafc",
...
function renderPageList() {
  pageList.innerHTML = state.pages.map((page) => `
    <div class="page-item">
      <button class="open-page ${page.id === state.currentPageId ? "is-active" : ""}" data-page="${page.id}">
        ${escapeHtml(page.name)}
      </button>
      <button class="delete-page" data-delete-page="${page.id}" title="Supprimer">×</button>
    </div>
  `).join("");
  ...
  pageList.querySelectorAll("[data-delete-page]").forEach((button) => {
...
function buildExport() {
  ...
  return state.pages.map((page, index) => {
    const fileName = index === 0 ? "index.html" : `${slugify(page.name)}.html`;
    return `<!-- FILE: ${fileName} -->\n${buildPageExport(page)}`;
  }).join("\n\n");
}

function buildPageExport(page) {
  const body = page.sections.map((section) => renderBlock(section)).join("\n");
  return `<!doctype html>
<html lang="fr">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>${escapeHtml(state.projectName)} - ${escapeHtml(page.name)}</title>
  <style>
    :root {
