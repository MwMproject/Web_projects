...
const ITEMS = [
  {
    id: "skin_neon_warrior",
    name: "SKIN NÉON RAVAGER",
    type: "skin",
    rarity: "epic",
    price: 800,
    icon: "🟠",
  ...
  {
    id: "skin_gold_warrior",
    name: "SKIN DORÉ RAVAGER",
    type: "skin",
    rarity: "legend",
    price: 1500,
    icon: "🌟",
...
function pickClass(cls) {
  ...
  btn.textContent =
    "► JOUER EN TANT QUE " +
    { warrior: "RAVAGER", mage: "HEXOMANCER", archer: "DRIFTER" }[cls];
}
...
function refreshHub() {
  ...
  const lvl = u.level || 1;
  document.getElementById("pcard-level").textContent =
    "⭐ NIVEAU " + lvl + " — " + getLevelTitle(lvl);
...
function launchGame() {
  const u = DB.get(CU.email);
  if (!u.class) {
    showNotif("Choisis une classe d'abord !", true);
    showRaw("s-class");
    return;
  }
  startGame(u.class);
...
function renderShop() {
  ...
  const items = ITEMS.filter(
    (i) => shopFilter === "all" || i.filter === shopFilter,
  );
...
function openBuyModal(id) {
  ...
  document.getElementById("modal-body").textContent =
    pendingItem.name + " — " + pendingItem.type;
  document.getElementById("modal-price").textContent =
    "◆ " + pendingItem.price + " crédits";
...
function confirmBuy() {
  if (!pendingItem) return;
  const u = DB.get(CU.email);
  if (u.coins < pendingItem.price) {
    showNotif("❌ Pas assez de crédits !", true);
    closeModal();
    return;
  }
  const inv = [...(u.inventory || [])];
  if (inv.includes(pendingItem.id)) {
    showNotif("⚠️ Déjà possédé !", true);
    closeModal();
    return;
  }
